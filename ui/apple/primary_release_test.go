...
func TestPrimaryAppleReleaseBumpsEveryShippingBuild(t *testing.T) {
	...
	if !strings.Contains(project, `MARKETING_VERSION: "1.0.11"`) {
		t.Fatal("review-corrected messaging build must use marketing version 1.0.11")
	}
	...
	if err != nil || build != 2608244 {
		t.Fatalf("review-corrected messaging build = %q, want 2608244", project[start:start+end])
	}
