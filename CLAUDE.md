...
## Development Rules
1. **Spec-driven**: Before building any module, write a spec in `specs/`. Spec includes: goal, approach, affected files, test criteria, rollback plan
2. **Test-first**: Every module needs tests. Use Vitest for TypeScript, pytest for Python
3. **Tool Registry is sacred**: Before building anything new, search `fort tools search` to check if existing tools handle the capability
