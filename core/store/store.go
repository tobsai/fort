...
`); err != nil {
	...
	if err := s.migrateAgentSourceInventory(); err != nil {
		return fmt.Errorf("store: migrate Agent Source inventory: %w", err)
	}
	if err := s.migrateChannelTurns(); err != nil {
		return fmt.Errorf("store: migrate channel turns: %w", err)
	}
